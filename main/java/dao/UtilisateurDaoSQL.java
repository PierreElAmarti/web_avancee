...
public class UtilisateurDaoSQL implements UtilisateurDao
{
	...
	private static DAOMapper daoMapperUtilisateur = new DAOMapper().setClass(Utilisateur.class).setAttribute("id", "id")
			.setAttribute("nomUtilisateur", "nomUtilisateur").setAttribute("adresseMail", "adresseMail")
			.setAttribute("motDePasse", "motDePasse").setAttribute("elo", "elo")
			.setAttribute("questionSecrete", "questionSecrete").setAttribute("reponseSecrete", "reponseSecrete")
			.setAttribute("permission", "permission");

	UtilisateurDaoSQL(DAOFactory daoFactory)
	{
	...
	public Long classement(Utilisateur utilisateur) throws DAOException
	{
		...
		try
		{
			 ...
			 */
			preparedStatement = DAOUtils.initialisationRequetePreparee(connexion, SQL_SELECT_CLASSEMENT, false,
					utilisateur.getElo());
	...
	public Utilisateur getByUsernameOrMail(String eMailouNomUtilisateur) throws DAOException
	{
		...
		try
		{
			 ...
			 * uniquement un id) et exécution.
			 */
			preparedStatement = DAOUtils.initialisationRequetePreparee(connexion,
					SQL_SELECT_PAR_ADRESSEMAILOUNOMUTILISATEUR, false, eMailouNomUtilisateur, eMailouNomUtilisateur);
	...
	public boolean emailDejaPris(String eMail) throws DAOException
	{
		...
		try
		{
			 ...
			 * uniquement un id) et exécution.
			 */
			preparedStatement = DAOUtils.initialisationRequetePreparee(connexion, SQL_SELECT_PAR_ADRESSEMAIL, false,
					eMail);
			ResultSet resultSet = preparedStatement.executeQuery();
			/* Parcours de la ligne de données retournée dans le ResultSet */
			if (resultSet.next())
	...
	public boolean nomUtilisateurDejaPris(String nomUtilisateur) throws DAOException
	{
		...
		try
		{
			 ...
			 * uniquement un id) et exécution.
			 */
			preparedStatement = DAOUtils.initialisationRequetePreparee(connexion, SQL_SELECT_PAR_NOMUTILISATEUR, false,
					nomUtilisateur);
			ResultSet resultSet = preparedStatement.executeQuery();
			/* Parcours de la ligne de données retournée dans le ResultSet */
			if (resultSet.next())
	...
	public void modifier(Utilisateur utilisateur) throws DAOException
	{
		...
		try
		{
			...
			preparedStatement = DAOUtils.initialisationRequetePreparee(connexion, SQL_UPDATE, true,
					utilisateur.getNomUtilisateur(), utilisateur.getAdresseMail(), utilisateur.getMotsDePasse(),
					utilisateur.getElo(), utilisateur.getQuestionSecrete(), utilisateur.getReponseSecrete(),
	...
	public void supprimer(Utilisateur utilisateur) throws DAOException
	{
		...
		{
			connexion = daoFactory.getConnection();
			preparedStatement = DAOUtils.initialisationRequetePreparee(connexion, SQL_DELETE_PAR_ID, true,
					utilisateur.getId());
			int statut = preparedStatement.executeUpdate();
			if (statut == 0)
			{
